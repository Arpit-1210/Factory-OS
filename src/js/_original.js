...
const PASSWORDS={owner:'289503',supervisor:'sup@123',rm:'rm@123'};
const ROLE_ACCESS={
  ...
  if(email && email.includes('@') && fbEnabled && firebase.auth){
    ...
    firebase.auth().signInWithEmailAndPassword(email, pwd)
      .then(cred=>{
        ...
        let role = USER_ROLES[email.toLowerCase()] || 'supervisor';
        // Check custom claims or displayName for role
        currentRole = role;
...
function togAtt(id){const l=S.lab.find(l=>l.id===id);l.present^=1;if(!l.present){l.doingOT=false;l.otHours=0;}persist();renderAtt();}
...
function markAll(v){S.lab.forEach(l=>{l.present=!!v;if(!v){l.doingOT=false;l.otHours=0;}});persist();renderAtt();}
